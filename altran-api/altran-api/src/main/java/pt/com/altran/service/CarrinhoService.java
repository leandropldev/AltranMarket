...
public class CarrinhoService {
	...
	public CarrinhoDTO create(CarrinhoDTO dto) {
		CarrinhoEntity entity = DozerConverter.parseObject(dto, CarrinhoEntity.class);
		
		if(repository.findCarrinhoEntityById(dto.getId()) != null) 
			throw new RequestException("Já existe uma compra cadastrado com este id: " + dto.getId());
	...
	public CarrinhoDTO findById(long id) {
		CarrinhoEntity entity = repository.findById(id)
			.orElseThrow(() -> new ResourceNotFoundException("Nenhuma compra encontrada com o id " + id));
